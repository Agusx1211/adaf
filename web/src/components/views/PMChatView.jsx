...
export default function PMChatView() {
  ...
  useEffect(function () {
    apiCall('/api/config/profiles', 'GET', null, { allow404: true })
      .then(function (list) {
        ...
        setProfiles(procs);
        var pmProfile = procs.find(function (p) {
          return p.name.toLowerCase().indexOf('pm') === 0 || p.name.toLowerCase().indexOf('manager') >= 0;
        });
        if (pmProfile) {
          setSelectedProfile(pmProfile.name);
  ...
  var loadMessages = useCallback(async function () {
    ...
    setLoading(true);
    try {
      var data = await apiCall(base + '/pm/chat', 'GET', null, { allow404: true });
      setMessages(data || []);
    } catch (err) {
      if (!err.authRequired) console.error('Failed to load PM chat:', err);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(function () {
    ...
    try {
      ...
      console.error('PM Chat WebSocket error:', e);
      setSending(false);
      return;
    }
    ...
    ws.addEventListener('message', function (wsEvent) {
      try {
        ...
        if ((type === 'done' || type === 'loop_done') && !finalized) {
          ...
          // Save response with structured events preserved in the local message object
          saveAssistantResponse(finalText, events.slice());
          setSending(false);
        ...
        console.error('PM Chat WebSocket parse error:', e);
      }
    });

    ws.addEventListener('error', function () { setSending(false); });
    ws.addEventListener('close', function () { wsRef.current = null; });
  ...
  async function handleSend(e) {
    ...
    try {
      var result = await apiCall(base + '/pm/chat', 'POST', {
        message: msg,
  ...
  async function handleClear() {
    if (!window.confirm('Clear all PM chat history?')) return;
    try {
      await apiCall(base + '/pm/chat', 'DELETE');
      setMessages([]);
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-0)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 600, color: 'var(--text-1)' }}>PM Chat</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', color: 'var(--text-3)', padding: 60 }}>
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, opacity: 0.7, maxWidth: 400, margin: '0 auto', lineHeight: 1.6 }}>
              Ask about plans, issues, priorities, or project status. Select a profile above and type your message below.
            </div>
          </div>
        ) : (
      ...
      <div style={{
        ...
        <form onSubmit={handleSend} style={{
          ...
          <input
            ...
            placeholder={selectedProfile ? 'Ask about plans, issues, priorities...' : 'Select a profile first...'}
          ...
          {sending ? (
            ...
            <button
              type="submit"
              disabled={!input.trim() || !selectedProfile}
              style={{
                padding: '10px 16px',
                background: (!input.trim() || !selectedProfile) ? 'var(--bg-3)' : 'var(--accent)',
                border: 'none', borderRadius: 6,
                color: (!input.trim() || !selectedProfile) ? 'var(--text-3)' : '#000',
                fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 600,
                cursor: (!input.trim() || !selectedProfile) ? 'not-allowed' : 'pointer',
              }}
            >Send</button>
          )}
        </form>
